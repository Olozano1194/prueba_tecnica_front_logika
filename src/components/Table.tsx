...
const Table = <T,>({ data, columns, totalRow }: TableProps<T>) => {
  ...
  return (
    <main className="cards w-full flex flex-col justify-center items-center gap-y-4 rounded-xl">
      <div className="overflow-x-auto w-full">
        <table className="min-w-full border">
          ...
          <tbody className="text-sm text-gray-800 lg:text-lg">
            ...
            {totalRow && (
              <tr className="bg-slate-500 text-gray-300 font-bold md:text-lg">
                {table.getAllColumns().map((column) => {
                  const accessorKey = column.id;
                  const value =
                    totalRow[accessorKey as keyof typeof totalRow] ?? "";
